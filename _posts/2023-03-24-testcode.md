...
int main(void){
	...
	volatile int num_dm[10]={0x0001, 0x0002, 0x0004, 0x0008,
							 0x0010, 0x0020, 0x0040, 0x0080,
							 0x0100, 0x0200};
	volatile int sel_7s[4]={0b11110111, 0b11111011, 0b11111101, 0b11111110};
	volatile int num_7s[8]={0b00111111, 0b00000110, 0b01011011, 0b01001111,
	...
	volatile int mole[10]={0x0000};
	int mole_row=0, mole_column=0;
	
	volatile int prev=0x00, input=0x00, result=0x00;
	volatile int print[10]={0x0000};
	...
	for(int try=0; try<10; try++){
		do{
			mole_row = rand()%10;
			mole_column = rand()%10;
		}while(mole_row==hammer_row && mole_column==hammer_column);
			
		for(int i=0; i<10; i++) mole[i]=0x0000;	// initialize mole's location
		mole[mole_row]=num_dm[mole_column];
		// set mole location
		
		for(int sec=0; sec<4; sec++){		// mole appears for 5 seconds	
			for(int i=0; i<Time; i++){	// print dot matrix for 1 seconds
				prev=input;
				result=PosEdge(prev,&input);
				// detect positive edge
				...
				if(mole_row==hammer_row && mole_column==hammer_column){
					score++;
					for(int i=0; i<10; i++) mole[i]=0x0000;	// initialize mole's location
					mole_row=0; mole_column=0;
				}
				
				if(score>=7) break;
				
				SS_SEL=sel_7s[0];
				SS_DATA=num_7s[score];
